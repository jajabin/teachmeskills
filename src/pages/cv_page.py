...
def get_page_cv(self, method: str, endpoint: str, _qs) -> None:
    get_page(self, method, endpoint, "/cv", None, paths.CV_HTML)


def get_page_cv_education(self, method: str, endpoint: str, _qs) -> None:
    get_page(self, method, endpoint, "/cv/education", paths.CV_EDUCATION_JSON, paths.CV_EDUCATION_HTML)


def get_page_cv_job(self, method: str, endpoint: str, _qs) -> None:
    get_page(self, method, endpoint, "/cv/job", paths.CV_JOB_JSON, paths.CV_JOB_HTML)


def get_page_cv_skills(self, method: str, endpoint: str, _qs) -> None:
    get_page(self, method, endpoint, "/cv/skills", paths.CV_SKILLS_JSON, paths.CV_SKILLS_HTML)


def get_page_cv_projects(self, method: str, endpoint: str, _qs) -> None:
    get_page(self, method, endpoint, "/cv/projects", paths.CV_PROJECTS_JSON, paths.CV_PROJECTS_HTML)


def get_page_projects_editing(self, method: str, endpoint: str, _qs) -> None:
    get_page(self, method, endpoint, "/cv/projects/editing", paths.CV_PROJECTS_JSON, paths.CV_PROJECTS_EDITING_HTML)


def get_page(self, method: str, endpoint: str, redirect_to: str, file_content: Path, file_html: Path) -> None:
    switcher = {
        "GET": show_page_cv,
        "POST": save_page_cv,
    }
    if method in switcher:
        switcher[method](self, endpoint, redirect_to, file_content, file_html)
    else:
        raise errors.MethodNotAllowed


def show_page_cv(self, endpoint: str, _redirect_to, file_content: str, file_html: str) -> None:
    """
    show a page
...
def save_page_cv(self, endpoint: str, redirect_to: str, file_content: Path, _file_html) -> None:
    """
    save some settings
    :param self: server instance
    :param endpoint: current endpoint
...
def edit_project(self, _redirect_to, file_content: Path) -> None:
    new_project_content = uu.parse_user_sessions(self)
    ...
    responds.respond_302(self, "/cv/projects")


def add_project(self, _redirect_to, file_content: Path) -> None:
    new_project_content = uu.parse_user_sessions(self)
    projects_content = ju.read_json_file(file_content)
    new_project = {}
...
def remove_project(self, _redirect_to, file_content: Path) -> None:
    new_project_content = uu.parse_user_sessions(self)
    projects_content = ju.read_json_file(file_content)
